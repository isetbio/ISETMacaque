function spectrogramStruct = multiTaperSpectrogram(timeCourse, recordedTimeAxisSeconds)
% Usage simulator.analyze.multiTaperSpectrogram(timeCourse, recordedTimeAxisSeconds)
    
    
    maxTimeSeconds = recordedTimeAxisSeconds(end);
    dtSeconds = recordedTimeAxisSeconds(2)-recordedTimeAxisSeconds(1);

    interpolatedTimeAxisSeconds = 0:dtSeconds:maxTimeSeconds;
    interpolatedTimeCourse = interp1(recordedTimeAxisSeconds, timeCourse, interpolatedTimeAxisSeconds, 'linear');
    samplingFrequencyHz = 1.0/dtSeconds;
    ...
    [spec,stimes,sfreqs] = doIt(...
        interpolatedTimeCourse,  samplingFrequencyHz, ...
        [lowFrequency highFrequency], ...
        [taperParams.timeBandwidth, taperParams.tapersNum], ...
        [windowParams.lengthSeconds, windowParams.stepSizeSeconds]);

    spectrogramStruct = struct(...
        'power', spec', ...
        'temporalSupportSeconds', stimes, ...
        'frequencySupportHz', sfreqs);
end



function [mt_spectrogram,stimes,sfreqs] = doIt(varargin)
...
default={[],[],[0 varargin{2}/2],[5 9], [5 1], 0, false, false, true}; % updated on 05/18/2020 to remove floor on (Fs/2)
