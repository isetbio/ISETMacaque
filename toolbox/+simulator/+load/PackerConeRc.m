function d = PackerConeRc(eccSamples, varargin)
    
    p = inputParser;
    p.addParameter('eccDegsSupport', [], @(x)(isempty(x)||(isnumeric(x))));
    p.addParameter('generatePlot', false, @islogical);
    p.parse(varargin{:});
    eccDegsSupport = p.Results.eccDegsSupport;
    generatePlot = p.Results.generatePlot;

    [temporalMeridian, nasalMeridian] = macaqueConeDensity();
    temporalMeridian.coneSpacingMM = WatsonRGCModel.spacingFromDensity(temporalMeridian.coneDensityPerSquaredMM);
    nasalMeridian.coneSpacingMM = WatsonRGCModel.spacingFromDensity(nasalMeridian.coneDensityPerSquaredMM);
    
    micronsPerVisualDegree = 210;
    temporalMeridian.coneSpacingDegs = temporalMeridian.coneSpacingMM * 1e3/ micronsPerVisualDegree;
    nasalMeridian.coneSpacingDegs = nasalMeridian.coneSpacingMM * 1e3/ micronsPerVisualDegree;
   
    temporalMeridian.eccDegs = temporalMeridian.eccMM * 1e3/ micronsPerVisualDegree;
    nasalMeridian.eccDegs = nasalMeridian.eccMM * 1e3/ micronsPerVisualDegree;
    temporalMeridian.coneRcDegs = temporalMeridian.coneSpacingDegs * 0.204 * sqrt(2.0);
    nasalMeridian.coneRcDegs = nasalMeridian.coneSpacingDegs * 0.204 * sqrt(2.0);
    
    % Compute nasal and temporal meridian cone Rc data on the same eccDegs support
    if (isempty(eccDegsSupport))
        tabulatedEccDegs = logspace(log10(temporalMeridian.eccDegs(1)), log10(50), eccSamples);
    else
        tabulatedEccDegs = eccDegsSupport;
    end

    d.temporalConeRcDegs = interp1(temporalMeridian.eccDegs, temporalMeridian.coneRcDegs, tabulatedEccDegs);
    d.nasalConeRcDegs = interp1(nasalMeridian.eccDegs, nasalMeridian.coneRcDegs, tabulatedEccDegs);
    d.eccDegs = tabulatedEccDegs;
    ...
    if (generatePlot)
        ...
        figure(3); clf;
        plot(temporalMeridian.eccDegs, temporalMeridian.coneRcDegs, 'rs-');
        hold on;
        plot(nasalMeridian.eccDegs, nasalMeridian.coneRcDegs, 'bs-');
        legend({'temporal', 'nasal'});
        xlabel('eccentricity (degs)');
        ylabel('cone Rc (degs)');
        set(gca, 'XScale', 'log', 'XLim', [0.01 30], 'XTick', [0.01 0.03 0.1 0.3 1 3 10 30]);
        set(gca, 'YScale', 'log', 'YLim', [0.002 2], 'YTick', [0.003 0.01 0.03 0.1 0.3 1 3], 'YTickLabel', {'.003', '.01', '.03', '.1', '.3', '1', '3'});
