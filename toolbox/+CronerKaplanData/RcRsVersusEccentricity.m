...
%
    ...
    if (generateFigure) || (addRcRsRatioData)
        ...
        if (addRcDegsData) && (~isempty(extraRcDegsData.eccDegs))
            hold(ax, 'on');
            p = scatter(ax, extraRcDegsData.eccDegs, extraRcDegsData.values, 100, ...
        ...
        if (addRsDegsData) && (~isempty(extraRsDegsData.eccDegs))
            hold(ax, 'on');
            p = scatter(ax, extraRsDegsData.eccDegs, extraRsDegsData.values, 144, ...
...
%        
        ...
        if (addRcRsRatioData)
            hold(ax, 'on');
            scatter(ax, extraRcRsRatioData.eccDegs, extraRcRsRatioData.values, 100, ...
            'filled', 'MarkerEdgeColor', [1 0.8 0.2]*0.5, 'MarkerFaceColor', [1 0.8 0.2], ...
            'MarkerFaceAlpha', 0.5, 'LineWidth', 1.0);
        end
