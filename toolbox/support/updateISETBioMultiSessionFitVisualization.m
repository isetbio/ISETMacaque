function updateISETBioMultiSessionFitVisualization(visStruct, iRGCindex, iCone, ...
    ...
    for iPos = 1:size(fittedSTFs,2)

        % Plot the best fit so far
        ax = visStruct.axSTFfits{iPos};
        cla(ax);
        
        % Training session
        p0 = plot(ax, examinedSpatialFrequencies, squeeze(theTrainingSTFs(iPos,:)), 'k-', 'LineWidth', 1.5);
        hold(ax, 'on');

        % Cross-validated session 1 fitted STF with scaled STF from training model
        p1 = plot(ax, examinedSpatialFrequencies, squeeze(fittedSTFs(1, iPos,:)), 'r-', 'LineWidth', 1.5);
        
        % Cross-validated session 2 fitted STF with scaled STF from training model
        p2 = plot(ax, examinedSpatialFrequencies, squeeze(fittedSTFs(2, iPos,:)), 'b-', 'LineWidth', 1.5);
    
    
        % Training session measured STF
        plot(ax, examinedSpatialFrequencies, theTrainingSTFdata, 'ko', ...
            'MarkerSize', 14, 'MarkerFaceColor', [0.5 0.5 0.5], ...
            'LineWidth', 1.0);

        % Session 1 measured STF
        plot(ax, examinedSpatialFrequencies, theMeasuredSTFdata(1,:), 'ro', ...
            'MarkerSize', 14, 'MarkerFaceColor', [1 0.5 0.5], ...
            'LineWidth', 1.0);
    
       
        % Session 2 measured STF
        plot(ax, examinedSpatialFrequencies, theMeasuredSTFdata(2,:), 'bo', ...
            'MarkerSize', 14, 'MarkerFaceColor', [0.5 0.5 1.0], ...
            'LineWidth', 1.0);
        ...
        ylabel(ax, '\Delta F / F');
        legend(ax, [p0, p1, p2], ...
            {sprintf('training session %d', dTrainSession) ...
             sprintf('test session %d', dTestSession(1)) ...
             sprintf('test session %d', dTestSession(2))} ...
            );
        title(ax,sprintf('Fitting at position %d', iPos));
    end
...
%             sprintf('cv-RMSE map (range: %2.2f - %2.2f)', minRMSerror, maxRMSerror), ...
...
%             'FontWeight', 'normal', 'FontSize', 15);
%     else
%         title(visStruct.axRF, sprintf('Rs/Rc: %2.2f, Kc/Ks: %2.2f, cv-RMSE: %2.2f (train: %2.2f)', ...
%             RsDegs/RcDegs, 1./KsToKc, ...
%             rmsErrors(iRGCindex,minRMSerrorConeIndex), ...
