function updateISETBioFitVisualization(visStruct, iRGCindex, iCone, ...
    ...
    fittedParams, rmsErrors, examinedSpatialFrequencies, fittedSTFs, ...
    ...
    for iiCone = 1:iCone
        linePlotHandle = plot(visStruct.axFit, examinedSpatialFrequencies, squeeze(fittedSTFs(iRGCindex, iiCone,:)), ...
    ...
    plot(visStruct.axFit, examinedSpatialFrequencies, squeeze(fittedSTFs(iRGCindex, minRMSerrorConeIndex,:)), ...
        '-', 'LineWidth', 4.0, 'Color', [1.0 0.0 0.0]);
    plot(visStruct.axFit, examinedSpatialFrequencies, squeeze(fittedSTFs(iRGCindex, minRMSerrorConeIndex,:)), ...
        '-', 'LineWidth', 2, 'Color', [1 0.5 0.5]);
    ...
    title(visStruct.axFit, fitTitle, 'FontWeight', 'normal');
    ...
    for iiCone = 1:iCone
        if (iiCone ~= minRMSerrorConeIndex)
            ...
            % Opacity of lines : 0.5
            linePlotHandle2.Color = [linePlotHandle.Color 0.25];
        end
    end
    ...
    plot(visStruct.axRF, xMicrons, centerProfile-surroundProfile, 'k-', 'LineWidth', 2);
    ...
    title(visStruct.axRF, sprintf('Rs/Rc: %2.2f, Kc/Ks: %2.2f RMSE: %2.2f', ...
        RsDegs/RcDegs, 1./KsToKc, rmsErrors(iRGCindex,minRMSerrorConeIndex)), ...
        'FontWeight', 'normal');
