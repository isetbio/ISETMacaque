function [inSampleErrors,outOfSampleErrors, modelVariant] = batchFitISETBioModelToAOSTFdata(...
    ...
    tStarted = clock;
    [inSampleErrors,outOfSampleErrors] = doIt(crossValidateModel, crossValidateModelAgainstAllSessions, ...
...
function [inSampleErrors,outOfSampleErrors] = doIt(crossValidateModel, crossValidateModelAgainstAllSessions, ...
    trainModel, monkeyID, modelVariant , ...
    maxRecordedRGCeccArcMin, visualizedLocationsNum, ...
    targetLcenterRGCindices, targetMcenterRGCindices, ...
    startingPointsNum)
    ...
    if (~isempty(targetLcenterRGCindices))
        ...
        if (~crossValidateModel)
            ...
            if (trainModel)
                ...
                inSampleErrors = zeros(numel(dCrossValidatedData), numel(indicesOfModelConesDrivingLcenterRGCs));
                outOfSampleErrors = zeros(numel(dCrossValidatedData), numel(indicesOfModelConesDrivingLcenterRGCs),2);
                ...
                for iCrossValidationRun = 1:numel(dCrossValidatedData)
                    ...
                    % Save cross-validation errors for this run 
                    tmp = rmsErrorsLcenterRGCs{iCrossValidationRun};
                    outOfSampleErrors(iCrossValidationRun,:,:) = (squeeze(tmp(:, targetLcenterRGCindices,:)))';
                    tmp = rmsErrorsTrainLcenterRGCs{iCrossValidationRun};
                    inSampleErrors(iCrossValidationRun,:) = squeeze(tmp(targetLcenterRGCindices,:));
                end
            end
        ...
        save(theFitsFilename,...
            ...
            'indicesOfModelConesDrivingLcenterRGCs', ...
            'inSampleErrors', 'outOfSampleErrors', ...
            'startingPointsNum');
        ...
        if (~crossValidateModel)
            ...
            [fittedParamsMcenterRGCs, ...
             ...
             surroundConeIndicesMcenterRGCs, surroundConeWeightsMcenterRGCs ...
                            ...
                            'modelTransducerFunctionAccountsForResponseOffset');
            ...
            if (trainModel)  
                ...
                for iCrossValidationRun = 1:numel(dCrossValidatedData)
                    ...
                    % Save cross-validation errors for this run 
                    tmp = rmsErrorsMcenterRGCs{iCrossValidationRun};
                    outOfSampleErrors(iCrossValidationRun,:,:) = (squeeze(tmp(:, targetMcenterRGCindices,:)))';
                    tmp = rmsErrorsTrainMcenterRGCs{iCrossValidationRun};
                    inSampleErrors(iCrossValidationRun,:) = squeeze(tmp(targetMcenterRGCindices,:));
                end
            end
