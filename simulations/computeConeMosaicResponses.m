function computeConeMosaicResponses(monkeyID, apertureParams, coneCouplingLambda, opticalDefocusDiopters, PolansSubject, visualStimulus)
    ...
    if (isempty(PolansSubject))
        ...
        pupilDiameterMM = 3.0;
        PolansOptics.constants.subjectRequiresCentralRefractionCorrection(PolansSubject)
...
function [theScene, scalingFactor] = generateStimulus(visualStimulus,  wavelengthSupport, theOI, theContrast, theSpatialFrequency, theSpatialPhase, theScalingFactor)
    ...
    stimFrameParams = struct(...
                ...
                'sceneSpatialUpsampleFactor', 9, ... % actual stim pixel size too close to cone aperture, so upsample the scene to have a well-sampled cone aperture
